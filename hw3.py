...
def lin_comb_mat_vec_mult(M, v):
    ...
    return sum(v[i]*matcolumn[i] for i in matcolumn.keys())
...
def lin_comb_vec_mat_mult(v, M):
    ...
    return sum(v[i]*matrow[i] for i in matrow.keys())
...
def Mv_mat_mat_mult(A, B):
    ...
    B_columns = mat2coldict(B)
    return coldict2mat({ i:A*B_columns[i] for i in B_columns.keys() } )
...
def vM_mat_mat_mult(A, B):
    ...
    return rowdict2mat({ i:A_rows[i]*B for i in A_rows.keys() } )
