...
def vec_select_sum(veclist, k, D): 
    ...
    selected_veclist = vec_select(veclist, k)
    return vec_sum(selected_veclist, D)
...
def scale_vecs(vecdict):
    ...
    return [ (1/i)*vecdict[i] for i in vecdict.keys() ]
...
def GF2_span(D, L):
    ...
    return [ x[0]*L[0] + x[1]*L[1] for x in product([0,1],repeat=len(L)) ]
