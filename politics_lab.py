...
def create_voting_dict():
    ...
    for i in voting_data:
        key = i.split()[0]
        dictt.update({key:[int(x) for x in i.split()[3:]]})
    return dictt
...
def most_similar(sen, voting_dict):
    ...
    for i in voting_dict.keys():
        element_distance = policy_compare(sen,i, voting_dict) 
        if i!=sen and (first==0 or element_distance>=near):
            first = 1
            near = element_distance
            senator = i
...
def find_average_record(sen_set, voting_dict):
    ...
    for i in sen_set:
        if first==0: 
            sum_aux = voting_dict[i]
            first = 1
        else:
            sum_aux = [ x+y for x,y in zip(sum_aux,voting_dict[i]) ]
    sum_aux = [ x/len(sen_set) for x in sum_aux ] 
...
def bitter_rivals(voting_dict):
    ...
    first = 0
    min_agree = 0
    pairs_sen = list(itertools.combinations(voting_dict.keys(),2))
    for i in pairs_sen:
        agree = policy_compare(i[0], i[1], voting_dict)
        if first==0 or agree<min_agree:
            first = 1
            min_agree = agree
            senator_A = i[0]
            senator_B = i[1]
